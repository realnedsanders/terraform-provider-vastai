...
// TestNetworkVolumeResource_Schema_OfferID_OptionalComputed verifies offer_id is Optional+Computed with AtLeast(1) validator.
// offer_id is a creation-time attribute not returned by the API, so it uses Optional+Computed with UseStateForUnknown.
func TestNetworkVolumeResource_Schema_OfferID_OptionalComputed(t *testing.T) {
	r := NewNetworkVolumeResource()
	req := resource.SchemaRequest{}
	var resp resource.SchemaResponse
	r.Schema(context.Background(), req, &resp)
...
// TestNetworkVolumeResource_Schema_Size_OptionalComputed verifies size is Optional+Computed, RequiresReplace, AtLeast(1).
// size is a creation-time attribute not returned by the API, so it uses Optional+Computed with UseStateForUnknown.
func TestNetworkVolumeResource_Schema_Size_OptionalComputed(t *testing.T) {
	r := NewNetworkVolumeResource()
	req := resource.SchemaRequest{}
	var resp resource.SchemaResponse
	r.Schema(context.Background(), req, &resp)
